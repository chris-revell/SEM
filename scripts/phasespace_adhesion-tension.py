...
for measurement in measurements:
    ...
    for i in enumerate(datafolders):
        ...
        if "neighbours" in measurement:
            z = measurementdata[-1,1] #np.mean(measurementdata[:,1])
        else:
            z = measurementdata[-1,3] #np.mean(measurementdata[:,3])
        ...
        if "neighbours" in measurement:
            ax.set_title("Final value of number of additional epiblast-epiblast neighbour pairs\nrelative to random system, averaged over time")
        if "surface" in measurement:
            ax.set_title("Final value of proportion of external surface occupied by\nprimitive endoderm, averaged over time")
        if "radius_time" in measurement:
            ax.set_title("Final value of distance of primitive endoderm from centre of mass\nrelative to random system, averaged over time")
        ...
        plt.close(fig)
        np.savetxt(os.path.join(argv[1],measurement[:-4]+"_phasespace_data_finalvalue.txt"),phasespace)
