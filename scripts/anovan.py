...
def anovanall(runfolder):
    eng = matlab.engine.start_matlab()

    folderstoignore = ["p_values","meandata","meanplots"]
    datafolders = [os.path.join(runfolder,f) for f in os.listdir(runfolder) if os.path.isdir(os.path.join(runfolder, f)) and f not in folderstoignore]

    timepointlabels = np.array([1,2,3,4,5,6])

    if os.path.exists(os.path.join(runfolder,"p_values")):
        pass
    else:
        os.mkdir(os.path.join(runfolder,"p_values"))
    ...
    for measurement in measurements:
        ...
        outfile = open(os.path.join(runfolder,"p_values",measurement[:-4]+"_p.txt"),'w')
        #outfile.write("time, p = "+str(p_out[0])+"\ncell type,p = "+str(p_out[1]))
        outfile.write(str(p_out[1]))

        #np.savetxt(os.path.join(runfolder,"anova/data.txt"),data)
    ...
    if os.path.exists(os.path.join(datafolders[0],"stats/neighbours_dif.txt")):
        ...
        np.savetxt(os.path.join(runfolder,"anova/neighbours_dif.txt"),neighbours_dif)
    """
